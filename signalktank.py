...
def updateTanks():
    for path in SIGNALK_TANK_PATH_TO_SERVICE:
        h1 = http.client.HTTPConnection(SIGNALK_SERVER)																   
        h1.request('GET', path)													  
        res = h1.getresponse()																						
        if res.status == 200:																									   
...
def main():
    ...
    if not SIGNALK_TANKS:
        h1 = http.client.HTTPConnection(SIGNALK_SERVER)
        h1.request('GET', SIGNALK_SELF_PATH + '/tanks/')
        res = h1.getresponse()
        if res.status == 200:
    ...
    if SIGNALK_TANKS:
        for tank in SIGNALK_TANKS:
            if ('path' in tank):
                ...
                h1 = http.client.HTTPConnection(SIGNALK_SERVER)
                h1.request('GET', SIGNALK_SELF_PATH + '/' + tank['path'])
                res = h1.getresponse()
                if res.status == 200:
                    jsondata = json.loads(res.read())
                    fluidType = SIGNALK_TO_N2K_FLUID_TYPES[fluidtype]
        ...
        GLib.timeout_add(UPDATE_INTERVAL, updateTanks)
